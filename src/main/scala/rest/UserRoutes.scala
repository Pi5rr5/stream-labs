...
class UserRoutes(modules: Configuration with PersistenceModule with DbModule with ActorModule) extends Directives {
  ...
  def userGetRoute = path("users" / IntNumber) { (id) =>
    get {
      validate(id > 0, "{ error: 'The supplier id should be greater than zero !' }") {
  ...
  @ApiOperation(value = "Update User", notes = "", nickname = "", httpMethod = "PATCH", produces = "text/plain")
  @ApiImplicitParams(Array(
    new ApiImplicitParam(name = "body", value = "User Object", required = true,
  ...
  def userUpdateRoute = path("users") {
    patch {
  ...
  val routes: Route = usersGetRoute ~ userGetRoute ~ userPostRoute ~ userUpdateRoute ~ userDeleteRoute
